...
bl_info = {
    ...
    "blender": (2, 6, 4),
    "location": "User Preferences > Input",
    "category": "System"
}
...
# TODO:
#   - 'Sculpt/Paint pan' modal, special panning that positions target
#       focus automatically, centered to volumes one is moving around
#   - 'Select Brush by Name' configurable hotkey
#   - 'Linear Transform' operator, create temporary bone and deform mesh;
#       will have nice falloff, symmetry options, translate/rotate/scale
#   - Sculpting silhouette mode
...
class ClearTransMenu(bpy.types.Menu):
...
class SculptBrushDirectionToggle(bpy.types.Operator):
...
class BrushPressureToggle(bpy.types.Operator):
...
class MergeContextSensitive(bpy.types.Operator):
...
class DeleteContextSensitive(bpy.types.Operator):
...
# Possible types: VERT, EDGE, FACE, ALL, EDGE_FACE, ONLY_FACE, EDGE_LOOP
class DeleteContextSensitiveSpecial(bpy.types.Operator):
...
def register():
    ...
    bpy.utils.register_class(ClearTransMenu)
    bpy.utils.register_class(MergeContextSensitive)
    bpy.utils.register_class(SculptBrushDirectionToggle)
    bpy.utils.register_class(BrushPressureToggle)
    bpy.utils.register_class(DeleteContextSensitive)
    bpy.utils.register_class(DeleteContextSensitiveSpecial)

def unregister():
    bpy.utils.unregister_class(ClearAllTransforms)
    bpy.utils.unregister_class(ClearTransMenu)
    bpy.utils.unregister_class(MergeContextSensitive)
    bpy.utils.unregister_class(SculptBrushDirectionToggle)
    bpy.utils.unregister_class(BrushPressureToggle)
    bpy.utils.unregister_class(DeleteContextSensitive)
    bpy.utils.unregister_class(DeleteContextSensitiveSpecial)
#
